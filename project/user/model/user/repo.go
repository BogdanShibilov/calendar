...
func ByUsername(ctx context.Context, username string) (*User, error) {
	var user User
	err := pgStorage.GetContext(ctx, &user, "SELECT id, username, password_hash, created_at, updated_at FROM users WHERE username = $1", username)
	if err != nil {
		if errors.Is(err, sql.ErrNoRows) {
			return nil, storage.ErrNotFound
		}
